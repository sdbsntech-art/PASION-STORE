...
export default function App() {
  ...
  const [adminTab, setAdminTab] = useState('products'); // products, orders, security, settings
  ...
  const handleAdminLogin = (e) => {
    e.preventDefault();
    if (loginPasscode === adminSettings.passphrase) {
      setIsAdmin(true);
      localStorage.setItem('passion_store_is_admin', 'true');
      setLoginPasscode('');
      setLoginError('');
      showNotification("🔓 Authentification réussie. Bienvenue dans la console Admin !");
    } else {
      setLoginError("Passphrase incorrecte. Accès refusé.");
      showNotification("❌ Tentative d'accès non autorisée détectée.");
    }
  };
      ...
      <main className="main-content">
        ...
        <section className="hero-banner glass animate-fade-in-up">
          <div className="pulse-security">
            ...
            <span>PROTECTION CONTRE LE PLAGIAT ACTIVER</span>
          </div>
          <h1 className="hero-title">{adminSettings.storeName}</h1>
          <p className="hero-subtitle">
            Découvrez nos collections haut de gamme : accessoires technologiques exclusifs, ordinateurs d'élite, montres connectées haut de gamme et livres de référence.
          </p>
        </section>
          ...
          <div className="modal-content glass" style={{ maxWidth: adminTab === 'products' || adminTab === 'orders' ? '960px' : '640px' }}>
            ...
            {!isAdmin ? (
              <div className="modal-body" style={{ padding: '3rem 2rem', textAlign: 'center' }}>
                ...
                <form onSubmit={handleAdminLogin} style={{ maxWidth: '360px', margin: '0 auto' }}>
                  <div className="form-group">
                    <input 
                      type="password" 
                      value={loginPasscode}
                      onChange={(e) => setLoginPasscode(e.target.value)}
                      placeholder="Passphrase d'administration"
                      className="form-input"
                  ...
                  {adminTab === 'settings' && (
                    <form onSubmit={handleSaveSettings}>
                      <div style={{ marginBottom: '1.5rem' }}>
                        ...
                        <p className="text-secondary" style={{ fontSize: '0.8rem' }}>Personnalisez les coordonnées et credentials de Passion Store.</p>
                      </div>
                      ...
                      <div className="form-group">
                        <label className="form-label">Passphrase d'Administration (Accès console)</label>
                        <input 
                          type="text" 
                          value={adminSettings.passphrase}
                          onChange={(e) => setAdminSettings({...adminSettings, passphrase: e.target.value})}
                          className="form-input"
                          required
                        />
                      </div>
          ...
          <div className="modal-content glass">
            ...
            <div className="modal-header">
              <h2 style={{ fontSize: '1.5rem' }}>
                {editingProduct ? `Modifier le produit: ${editingProduct.name}` : 'Ajouter un nouveau produit'}
              </h2>
            </div>
            ...
            <form onSubmit={handleProductFormSubmit} className="modal-body">
              <div className="form-group" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <label className="form-label">Nom complet du produit *</label>
                  <input 
                    type="text" 
                    value={productForm.name}
                    onChange={(e) => setProductForm({...productForm, name: e.target.value})}
                    className={`form-input ${productFormErrors.name ? 'form-input-error' : ''}`}
                    placeholder="Ex: Apple iPad Pro M4"
                    required
                  />
              ...
              <div className="form-group" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
                <div>
                  <label className="form-label">Marque / Auteur *</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="form-label">Modèle / Éditeur</label>
                  <input 
                    type="text" 
                    value={productForm.model}
                    onChange={(e) => setProductForm({...productForm, model: e.target.value})}
                    className="form-input"
                    placeholder="Ex: Late 2024"
                  />
                </div>
                <div>
              ...
              <div className="form-group">
                ...
                <textarea 
                  ...
                  rows="3"
                  placeholder="Présentez le produit en quelques lignes..."
                  required
                ></textarea>
              ...
              <div className="form-group">
                ...
                <textarea 
                  ...
                  className="form-input"
                  rows="4"
                  placeholder="Processeur: Apple M4&#10;Mémoire RAM: 8 Go&#10;Stockage: 256 Go SSD"
                ></textarea>
              </div>
              ...
              <div className="form-group" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '1rem' }}>
                ...
                <label htmlFor="inStockCheck" style={{ cursor: 'pointer', fontSize: '0.9rem' }}>
                  Marquer cet article comme <strong>En Stock</strong>
                </label>
              </div>
